...
namespace 扑克牌
{
	...
	public partial class MainForm : Form
	{
		...
		card 黑桃J=new card("img/黑桃J.png",10);
		card 黑桃Q=new card("img/黑桃Q.png",10);
		card 黑桃K=new card("img/黑桃K.png",10);
		...
		card 红桃J=new card("img/红桃J.png",10);
		card 红桃Q=new card("img/红桃Q.png",10);
		card 红桃K=new card("img/红桃K.png",10);
		...
		card 方片J=new card("img/方片J.png",10);
		card 方片Q=new card("img/方片Q.png",10);
		card 方片K=new card("img/方片K.png",10);
		...
		card 梅花J=new card("img/梅花J.png",10);
		card 梅花Q=new card("img/梅花Q.png",10);
		card 梅花K=new card("img/梅花K.png",10);
		...
		void MainFormLoad(object sender, EventArgs e)
		{
			loading_instack();
			changecard();
			MessageBox.Show("如果您是第一次游玩，建议点击主窗口左上的帮助按钮，获取游戏规则说明","提示",MessageBoxButtons.OK,MessageBoxIcon.Information);
			for (int i = 0; i <10; i++) {
				table[i]=空牌;
			}
			cardload();
		...
		int goalcol()//抽牌后判断是否超过21点进行返回差值。
		{
		 int i=0;
		...
		if (i==21) {
				MessageBox.Show("你终于凑够了了21点,即将开始下一局","恭喜你");
			for (int j = 0; j <10; j++) {
		...
		goal.Text=Convert.ToString(Convert.ToInt32(goal.Text)-5);
		if(isItGameOver()){
			ENDING(true);
		   	}
		return;
